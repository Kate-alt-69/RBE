...
def one(old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"expected one ContainerClient anchor, found {count}: {old[:140]!r}")
...
# All three public operations must consume the same typed identity tuple.
for signature in [
    "pub async fn register_capability_manifest(",
    "pub async fn execute(",
    "pub async fn execute_and_wait(",
    ...
    if "identity: ContainerExecutionIdentity<'_>" not in block:
        raise SystemExit(f"typed Container identity missing from {signature}")
    for loose in ["runtime_image: &str", "source_id: &str", "environment: &str"]:
        if loose in block:
            raise SystemExit(f"loose identity field survived in {signature}: {loose}")
