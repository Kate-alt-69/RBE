...
if 'fancy-regex = "=0.16.2"' not in text:
    if 'regex = "1"\n' not in text:
        raise SystemExit("route-engine regex dependency anchor missing")
    text = text.replace('regex = "1"\n', 'regex = "1"\nfancy-regex = "=0.16.2"\n', 1)
cargo.write_text(text, encoding="utf-8")

modules = "engine/crates/route-engine/src/modules.rs"
old = r'''const REGX_MAX_PATTERN_BYTES: usize = 4096;
...
'''
new = r'''const REGX_MAX_PATTERN_BYTES: usize = 4096;
const REGX_MAX_INPUT_BYTES: usize = 1024 * 1024;
...
'''
replace_once(modules, old, new, "advanced REGX runtime")
...
tests = r'''    #[test]
    fn regx_descriptors_cover_common_api_validation_without_raw_patterns() {
        ...
        assert!(matches!(
            registry
                .call("regx", "test", &[Value::String("Good_123".into()), descriptor.clone()])
                .unwrap(),
            Value::Bool(true)
        ));
        assert!(matches!(
            registry
                .call("regx", "test", &[Value::String("admin_123".into()), descriptor])
                .unwrap(),
            Value::Bool(false)
        ));
    }
        ...
        assert!(matches!(
            registry
                .call(
                    "regx",
                    "raw",
                    &[Value::String(password.into()), Value::String("GoodPass1!".into())],
                )
                .unwrap(),
            Value::Bool(true)
        ));
        ...
        assert!(matches!(
            registry
                .call(
                    "regx",
                    "raw",
                    &[Value::String(repeated.into()), Value::String("the the".into())],
                )
                .unwrap(),
            Value::Bool(true)
        ));
    ...
    fn unknown_private_function_is_reported() {
'''
replace_once(modules, anchor, tests, "REGX tests")

docs = Path("doc/field-manager.md")
text = docs.read_text(encoding="utf-8")
old_docs = "The resolver is compiled as REL but remains pure. `regx.test(pattern, value)` provides bounded deterministic regex matching and `regx.raw(pattern)` validates/returns a raw regex pattern."
new_docs = "The resolver is compiled as REL but remains pure. `regx.test(pattern, value)` keeps ordinary regexes on the fast linear engine. `regx.test(value, descriptor)` provides readable validation descriptors (`allow`, `require`, `min`, `max`, `exclude`, `excludeContains`, `noConsecutive`, `ignoreCase`). `regx.raw(pattern)` validates advanced syntax, while `regx.raw(pattern, value)` uses a separately bounded backtracking engine for lookarounds/backreferences. `regx` is a shared REL builtin available to `.route`, `.module`, `.service`, `server.server`, and `.field`; Field REL remains restricted to the pure `math` + `regx` capability set."
if old_docs in text:
