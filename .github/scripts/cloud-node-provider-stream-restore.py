...
old_import = "use reqwest::header::{HeaderName, HeaderValue, AUTHORIZATION, CONTENT_TYPE, HOST};"
if old_import not in p:
    raise SystemExit("provider header import anchor missing")
p = p.replace(
    old_import,
...
old_constant = 'const HTTP_HEADER_VALUE_ENV: &str = "RBE_CN_PROV_HTTP_HEADER_VALUE";\n'
if old_constant not in p:
    raise SystemExit("provider error cap anchor missing")
p = p.replace(
    old_constant,
    old_constant + "const MAX_PROVIDER_ERROR_BYTES: usize = 1024;\n",
    1,
)

p = sub_once(
    r"self\.aws_request\(Method::GET,\s*&key,\s*Vec::new\(\),\s*None\)",
    "self.aws_request(Method::GET, &key, Vec::new(), None, None)",
    p,
    "AWS GET call",
)
p = sub_once(
    r"self\.aws_request\(Method::PUT,\s*&key,\s*bytes,\s*Some\(content_type\)\)",
...
'''
p = sub_once(
    r"(?m)^    pub async fn put\(",
    open_download + "    pub async fn put(",
    p,
    "open download insertion",
)

p = sub_once(
    r"(?s)(    async fn aws_request\(\s*&self,\s*method: Method,\s*key: &str,\s*body: Vec<u8>,\s*content_type: Option<&str>,)(\s*\) -> anyhow::Result<reqwest::Response> \{)",
    r"\1\n        range: Option<&str>,\2",
    p,
    "AWS range signature",
)
p = sub_once(
    r"(?s)(        if let Some\(content_type\) = content_type \{\s*request = request\.header\(CONTENT_TYPE, content_type\);\s*\}\s*)(        if !body\.is_empty\(\) \{)",
    r"\1        if let Some(range) = range {\n            request = request.header(RANGE, range);\n        }\n\2",
    p,
    "AWS range request header",
)

response_helpers = '''async fn checked_download_response(
    ...
    if start > 0 && status == StatusCode::PARTIAL_CONTENT {
        let content_range = response
            ...
            .to_str()
            .map_err(|error| {
                anyhow::anyhow!("Cloud Node provider returned invalid Content-Range header: {error}")
            })?;
...
'''
p = sub_once(
    r"(?s)^async fn response_bytes\(.*?^fn add_header\(",
    lambda _match: response_helpers + "fn add_header(",
    p,
...
restore = '''async fn restore_resource(
...
'''
s = sub_once(
    r"(?s)^async fn restore_resource\(.*?^fn resource_record\(",
    lambda _match: restore + "fn resource_record(",
    s,
    "streamed provider restore",
    re.MULTILINE,
)
