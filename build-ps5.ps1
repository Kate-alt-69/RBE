...
.DESCRIPTION
    ...
    Other flags:
        ...
        --dev-content       copy this repo's dev api/module content into
                                                 dist\<target>\ instead of empty scaffolding
                                                 folders (see build.ps1's matching flag)
...
# Compatibility for PowerShell 5.1: assume Windows host (PS5 runs on Windows).
# For PS5 we remove the pwsh-only version check and force OS flags used
# elsewhere in the script so behaviour matches `build.ps1` as closely as
# possible while remaining PS5-compatible.
$IsWindows = $true
$IsMacOS = $false
...
function Get-RequestedArches {
    ...
    if ($arches.Count -eq 0) { $arches += (Get-HostArch) }
    return $arches
}

function Resolve-Target {
    param([string]$Os, [string]$Arch, [bool]$UseMusl)
    ...
    switch ($Os) {
        "windows" {
            switch ($Arch) {
                ...
                default { throw "windows: no target triple for architecture '$Arch'" }
            }
        }
        "linux" {
            $libc = if ($UseMusl) { "musl" } else { "gnu" }
            switch ($Arch) {
                "x64"   { return "x86_64-unknown-linux-$libc" }
                "x86"   { return "i686-unknown-linux-$libc" }
                "arm64" { return "aarch64-unknown-linux-$libc" }
                "armv7" {
                    if ($UseMusl) { return "armv7-unknown-linux-musleabihf" }
                    else { return "armv7-unknown-linux-gnueabihf" }
                }
                default { throw "linux: no target triple for architecture '$Arch'" }
            }
        }
        "macos" {
            switch ($Arch) {
                ...
                default { throw "macos: no target triple for architecture '$Arch' (macOS only ships x64 and arm64/Apple Silicon)" }
            }
        }
...
# Distro/alias mapping (same as build.ps1)
function Get-TargetsForDistro {
    ...
    switch ($d) {
        ...
        'arch' | 'manjaro' | 'artix' { if ($arch -eq 'x86') { return @('i686-unknown-linux-gnu') } if ($arch -eq 'arm64') { return @('aarch64-unknown-linux-gnu') } return @('x86_64-unknown-linux-gnu') }
        'opensuse' | 'suse' | 'tumbleweed' | 'sles' { return @('x86_64-unknown-linux-gnu','aarch64-unknown-linux-gnu') }
        'alpine' | 'musl' | 'void-musl' { if ($arch) { switch ($arch) { 'x64' { return @('x86_64-unknown-linux-musl') } 'x86' { return @('i686-unknown-linux-musl') } 'arm64' { return @('aarch64-unknown-linux-musl') } 'armv7' { return @('armv7-unknown-linux-musleabihf') } } } return @('x86_64-unknown-linux-musl','aarch64-unknown-linux-musl','i686-unknown-linux-musl','armv7-unknown-linux-musleabihf') }
        'gentoo' | 'slackware' | 'void' | 'nixos' | 'guix' { return @('x86_64-unknown-linux-gnu','aarch64-unknown-linux-gnu') }
        'raspbian' | 'raspios' | 'pi' | 'raspberry' { if ($arch -eq 'arm64') { return @('aarch64-unknown-linux-gnu') } return @('armv7-unknown-linux-gnueabihf') }
        'clear' | 'clearlinux' { return @('x86_64-unknown-linux-gnu') }
        'wsl' | 'windows-subsystem-for-linux' | 'chromeos' { if ($arch -eq 'arm64') { return @('aarch64-unknown-linux-gnu') } if ($arch -eq 'x86') { return @('i686-unknown-linux-gnu') } return @('x86_64-unknown-linux-gnu') }
...
function Get-BuiltBinaryPath {
    ...
    # PS 5.1's Join-Path only takes -Path/-ChildPath (two segments) —
    # no -AdditionalChildPath the way PS7+ has. [System.IO.Path]::Combine
    # is available in .NET Framework too and takes any number of
    # segments, so it's the actual PS5-safe way to do this.
    if (-not [string]::IsNullOrEmpty($env:CARGO_TARGET_DIR)) {
        return [System.IO.Path]::Combine($env:CARGO_TARGET_DIR, $Target, $profileDir, $fileName)
    }
    return [System.IO.Path]::Combine($WorkspaceDir, "target", $Target, $profileDir, $fileName)
}
...
foreach ($target in $targets) {
    ...
    # folders unless -DevContent was passed (see build.ps1's identical
    # comment for the full reasoning — this is the PS5-compatible twin
    # of that script).
    if ($DevContent) {
