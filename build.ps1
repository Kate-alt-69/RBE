...
function Remove-LegacyContainerSigningKey {
    ...
    if (Test-Path -LiteralPath $keyPath) {
        throw "Failed to remove legacy local signing credential: $keyPath"
    }
    Write-Host "Removed legacy local signing credential: $keyPath" -ForegroundColor DarkYellow
}

function Initialize-ContainerSigningKey {
    if (-not [string]::IsNullOrWhiteSpace($env:RBE_CONTAINER_SIGNING_PRIVATE_KEY)) {
        if ($env:RBE_CONTAINER_SIGNING_PRIVATE_KEY -notmatch '^[0-9a-fA-F]{64}$') {
            throw 'RBE_CONTAINER_SIGNING_PRIVATE_KEY must contain exactly 64 hexadecimal characters.'
        }
        Remove-LegacyContainerSigningKey
        Write-Host 'Using externally supplied RBE container signing key.' -ForegroundColor DarkGray
        return
    }
    ...
    if ($key -notmatch '^[0-9a-f]{64}$') {
        throw 'Failed to generate a valid ephemeral 32-byte container signing key.'
    }

    $env:RBE_CONTAINER_SIGNING_PRIVATE_KEY = $key
    $script:GeneratedContainerSigningKey = $true
