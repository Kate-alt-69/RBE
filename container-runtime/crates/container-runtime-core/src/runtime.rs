...
impl Runtime {
    ...
    pub fn submit_with_policy(
        ...
        if !payload.is_empty() {
            self.cache.put_artifact(artifact_hash.clone(), payload);
        }
    ...
    pub fn cancel(&self, execution_id: &str) -> bool {
        ...
        for environment in &self.environments {
            found |= environment.cancel_queued_by_string(execution_id);
        }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if let Ok(entries) = std::fs::read_dir("/sys/devices/system/cpu") {
            for entry in entries.flatten() {
                ...
                let package = std::fs::read_to_string(topology.join("physical_package_id")).ok().and_then(|v| v.trim().parse::<u32>().ok());
                let core = std::fs::read_to_string(topology.join("core_id")).ok().and_then(|v| v.trim().parse::<u32>().ok());
...
fn run_isolated_worker(task: &ExecutionTask) -> anyhow::Result<()> {
    #[cfg(target_os = "linux")]
    {
        let cgroup_root = PathBuf::from("/sys/fs/cgroup/rbe");
        let cgroup = CgroupHandle::create(&cgroup_root, &task.id.to_string(), task.limits)?;
        let program = std::env::current_exe()?;
        let fuel = task.limits.cpu_millis.max(1).saturating_mul(10_000);
        let args = vec![
            "--worker".to_string(),
            "--artifact".to_string(),
            task.artifact_hash.clone(),
            "--fuel".to_string(),
            fuel.to_string(),
            "--memory".to_string(),
            task.limits.memory_bytes.to_string(),
        ];
...
mod tests {
    ...
    #[test]
    fn default_workers_per_swamp_is_one() {
        assert_eq!(RuntimeConfig::default().workers_per_swamp, 1);
    }
