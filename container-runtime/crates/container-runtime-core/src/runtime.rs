...
impl Default for RuntimeConfig {
    fn default() -> Self {
        Self { swamps_per_environment: physical_core_count(), workers_per_swamp: 1, rebalance_interval_ms: 25 }
    }
}
...
impl Runtime {
    pub fn new(config: RuntimeConfig) -> Arc<Self> {
        ...
        let runner: Runner = {
            let cache = Arc::clone(&cache); let cancelled = Arc::clone(&cancelled);
            Arc::new(move |task| {
        ...
        let completion: Arc<dyn Fn(&ExecutionTask, u64, Result<(), String>) + Send + Sync + 'static> = {
            let cache = Arc::clone(&cache); let cancelled = Arc::clone(&cancelled); let journal = Arc::clone(&journal);
        ...
        let environments = EnvironmentId::ALL.into_iter().map(|id| EnvironmentRuntime::new(id, config.swamps_per_environment, config.workers_per_swamp, EnvironmentStorage { limit_bytes: DEFAULT_ENVIRONMENT_STORAGE_BYTES, ephemeral: true }, Arc::clone(&runner), Arc::clone(&completion))).collect();
        ...
        let weak = Arc::downgrade(&runtime); let interval = runtime.config.rebalance_interval_ms;
        thread::Builder::new().name("rbe-runtime-scheduler".to_string()).spawn(move || { while let Some(runtime) = weak.upgrade() { runtime.rebalance_once(); thread::sleep(Duration::from_millis(interval)); } }).expect("failed to start runtime scheduler");
        runtime
    }
    ...
    pub fn submit_with_policy(&self, environment: EnvironmentId, artifact_hash: impl Into<String>, cost: WorkCost, limits: ResourceLimits, sandbox: SandboxPolicy, work_ms: u64, payload: Vec<u8>) -> ExecutionId {
        let artifact_hash = artifact_hash.into(); if !payload.is_empty() { self.cache.put_artifact(artifact_hash.clone(), payload); }
        let id = ExecutionId::new(self.next_execution.fetch_add(1, Ordering::Relaxed));
        self.journal.append(JournalEvent { kind: "queued".into(), epoch_ns: id.epoch_ns(), sequence: id.sequence(), environment: environment.to_string(), artifact_hash: artifact_hash.clone(), cpu: cost.cpu, memory: cost.memory, io: cost.io, network: cost.network, work_ms, limit_cpu_millis: limits.cpu_millis, limit_memory_bytes: limits.memory_bytes, limit_disk_bytes: limits.disk_bytes, limit_network_bytes: limits.network_bytes, limit_max_processes: limits.max_processes, limit_max_file_descriptors: limits.max_file_descriptors, limit_wall_time_ms: limits.wall_time_ms });
    ...
    pub fn cancel(&self, execution_id: &str) -> bool { let mut found = false; { let mut queue = self.global_queue.lock().expect("global queue poisoned"); let before = queue.len(); queue.retain(|(_, task)| task.id.to_string() != execution_id); found |= before != queue.len(); } for environment in &self.environments { found |= environment.cancel_queued_by_string(execution_id); } self.cancelled.lock().expect("cancel table poisoned").insert(execution_id.to_string()); self.journal.append_cancel_string(execution_id); found || true }
    pub fn restart_environment(&self, id: EnvironmentId) -> usize { let pending = self.environment(id).restart(); let count = pending.len(); if count != 0 { let mut queue = self.global_queue.lock().expect("global queue poisoned"); for task in pending { queue.push_back((id, task)); } } *self.generations.lock().expect("generation table poisoned").entry(id).or_default() += 1; count }
    ...
    pub fn rebalance_once(&self) { let pending = { let mut queue = self.global_queue.lock().expect("global queue poisoned"); queue.drain(..).collect::<Vec<_>>() }; for (environment, task) in pending { self.environment(environment).enqueue(task); } for environment in &self.environments { environment.rebalance(); } }
...
fn parse_execution_id(value: &str) -> Result<(u64, u64), ()> { let mut parts = value.strip_prefix("exec-").ok_or(())?.split('-'); let epoch_ns = u64::from_str_radix(parts.next().ok_or(())?, 16).map_err(|_| ())?; let sequence = u64::from_str_radix(parts.next().ok_or(())?, 16).map_err(|_| ())?; Ok((epoch_ns, sequence)) }
