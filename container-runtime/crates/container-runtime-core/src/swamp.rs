...
impl Swamp {
    pub fn new(
        ...
        let workers = (0..worker_count.max(1))
            .map(|worker_id| Worker::new(worker_id, Arc::clone(&on_complete)))
            .collect::<Vec<_>>();
    ...
    pub fn queued(&self) -> usize {
        self.queue.lock().expect("swamp queue poisoned").len()
    }

    pub fn queued_cost(&self) -> u64 {
        self.queue
            .lock()
            .expect("swamp queue poisoned")
            .iter()
            .map(|task| task.declared_cost.scalar())
            .sum()
    }
