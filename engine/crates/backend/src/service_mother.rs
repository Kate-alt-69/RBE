...
fn incompatible_service_runtime_error(
    path: &Path,
...
fn validate_service_compatibility(
    ...
    if probe.catalog_fingerprint_abi != service_runtime::SERVICE_CATALOG_FINGERPRINT_ABI_VERSION {
        mismatches.push(format!(
...
async fn probe_service_compatibility(service: &Path) -> anyhow::Result<()> {
    ...
    let result = tokio::time::timeout(SERVICE_COMPAT_TIMEOUT, async {
        ...
        .ok_or_else(|| {
            service_compat_protocol_error(
                service,
                "Service exited without a compatibility response",
            )
        })?;
        ...
        let probe: service_runtime::ServiceCompatibilityProbe = serde_json::from_str(line.trim())
            .map_err(|error| {
            service_compat_protocol_error(
                service,
...
mod tests {
    ...
    fn compatibility_probe_distinguishes_protocol_and_runtime_mismatches() {
        ...
        assert!(rendered
            .contains("https://kastrick.vercel.app/project/rbe/doc/error-codes/service#svc5003"));
