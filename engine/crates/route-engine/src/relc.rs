        ...
        match import_base(import) {
            ...
            ImportTarget::Service(service) => {
                ...
                let unit = compiled.get(target.id()).ok_or_else(|| {
                    RelcError::Link(format!(
                        "service `{service}` was registered but not compiled"
                    ))
                })?;
...
mod tests {
    ...
    fn typed_host_requirements_ignore_local_builtins() {
        ...
        assert!(
            requirements.contains(&RuntimeCapabilityRequirement::PublicHttp {
                operation: "get".into(),
            })
        );
        ...
        assert_eq!(
            requirements.len(),
            2,
            "json is local and needs no host grant"
        );
    }
    ...
    fn module_host_requirements_propagate_to_route_callers() {
        ...
        assert!(image.capability_requirements(route).unwrap().contains(
    ...
    fn service_namespace_import_expands_exported_operations() {
        ...
        let module = image
            ...
            .find(|id| {
                image
                    .source(id)
                    .is_some_and(|source| source.logical_name == "client")
            })
            .unwrap();
        let requirements = image.capability_requirements(module).unwrap();
        for operation in ["find", "remove"] {
            assert!(
                requirements.contains(&RuntimeCapabilityRequirement::Service {
                    service: "search".into(),
