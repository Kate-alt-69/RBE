        ...
        .collect::<Option<Vec<_>>>()
    {
        ...
        let args = if host.kind == ContainerCapabilityKind::Storage && host.operation == "write" {
            ...
            host_args
                ...
                .collect::<Option<Vec<_>>>()
                .ok_or_else(|| {
                    "native linked Module host arguments must resolve to static JSON values"
                        .to_string()
                })?
        };
...
fn lower_dynamic_storage_write_args(
    ...
    let suffix = format!(r#", "encoding":{encoding_json},"level":{level}}}]"#).into_bytes();
    ...
    let max_body_bytes = CONTAINER_MAX_CAPABILITY_PAYLOAD_BYTES
        ...
        .ok_or_else(|| {
            "dynamic storage.write static envelope exceeds capability limit".to_string()
        })?;
...
fn encode_json_body_storage_write_module(
    ...
    let response_offset =
        (input_offset + CONTAINER_MAX_EXECUTION_INPUT_BYTES + suffix.len() + 15) & !15usize;
    let memory_bytes = response_offset + CONTAINER_MAX_CAPABILITY_PAYLOAD_BYTES;
