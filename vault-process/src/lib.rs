...
impl VaultClient {
    pub fn spawn(service_name: impl Into<String>, data_dir: &Path) -> anyhow::Result<Self> {
        ...
        thread::Builder::new()
            ...
            .spawn(move || Worker::new(exe, service_name, data_dir, ready_tx).run(rx))?;

        match ready_rx.recv_timeout(STARTUP_TIMEOUT + Duration::from_secs(1)) {
            Ok(Ok(())) => Ok(Self { tx }),
            Ok(Err(err)) => Err(err),
            Err(mpsc::RecvTimeoutError::Timeout) => Err(anyhow::anyhow!(
                "Vault did not become ready within {:?}", STARTUP_TIMEOUT
            )),
    ...
    pub fn credential(&self, name: &str, caller: &str) -> anyhow::Result<secrecy::SecretString> {
        let (tx, rx) = mpsc::channel();
        self.tx.send(ClientCommand::Get {
            name: name.to_owned(), caller: caller.to_owned(), response: tx,
        }).map_err(|_| anyhow::anyhow!("Vault client worker is unavailable"))?;
        Ok(secrecy::SecretString::new(rx.recv()
    ...
    pub fn set_credential(&self, name: &str, value: &str, caller: &str) -> anyhow::Result<()> {
        let (tx, rx) = mpsc::channel();
        self.tx.send(ClientCommand::Set {
            name: name.to_owned(), value: value.to_owned(), caller: caller.to_owned(), response: tx,
        }).map_err(|_| anyhow::anyhow!("Vault client worker is unavailable"))?;
...
impl Worker {
    ...
    fn run(&mut self, rx: Receiver<ClientCommand>) {
        ...
        loop {
            match rx.recv_timeout(MONITOR_INTERVAL) {
                ...
                Err(mpsc::RecvTimeoutError::Timeout) => { if self.connection_is_dead() { let _ = self.restart_connection(); } }
                Err(mpsc::RecvTimeoutError::Disconnected) => { self.shutdown_child(); return; }
            }
        }
    }
    ...
    fn spawn_connection(&self, force_dbus: bool) -> anyhow::Result<Connection> {
        ...
        command.args([
            "--vault", "--separate-process", "--service-name", &self.service_name,
            "--data-dir",
        ]).arg(&self.data_dir);
        ...
        if line.is_empty() { let _ = child.kill(); let _ = child.wait(); return Err(anyhow::anyhow!("Vault exited before handshake")); }
        if serde_json::from_str::<NeedsDbus>(&line).map(|v| v.kind == "needs_dbus").unwrap_or(false) {
            let _ = child.kill(); let _ = child.wait(); return Err(anyhow::anyhow!("VAULT_NEEDS_DBUS"));
        }
        let ready: Ready = serde_json::from_str(&line).map_err(|e| anyhow::anyhow!("invalid Vault handshake: {e}"))?;
        if ready.kind != "ready" || ready.token.is_empty() { let _ = child.kill(); let _ = child.wait(); return Err(anyhow::anyhow!("Vault returned an invalid ready handshake")); }
        Ok(Connection { child, stdin, reader, token: ready.token, seq: 1 })
    }

    fn shutdown_child(&mut self) {
        if let Some(mut c) = self.connection.take() { let _ = c.child.kill(); let _ = c.child.wait(); }
    }
}
    ...
    loop {
        ...
        let request: Request = match serde_json::from_str(&line) {
            ...
            Err(error) => {
                ...
                writeln!(writer, "{}", serde_json::to_string(&response)?)?; writer.flush()?; continue;
            }
        };
