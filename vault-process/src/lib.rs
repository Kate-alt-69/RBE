...
impl VaultClient {
    pub fn spawn(service_name: impl Into<String>, data_dir: &Path) -> anyhow::Result<Self> {
        ...
        // Two-stage startup wait: the normal path gets the desired ~2 s
        // target, while a legitimate package/D-Bus bootstrap is allowed to
        // finish instead of being torn down mid-install. The API remains
        // unavailable until Vault is actually ready.
            ...
            Err(mpsc::RecvTimeoutError::Timeout) => {
                match ready_rx.recv_timeout(STARTUP_MAX.saturating_sub(STARTUP_TARGET)) {
                    ...
                    Err(mpsc::RecvTimeoutError::Disconnected) => Err(anyhow::anyhow!(
                        "Vault client worker exited during startup"
                    )),
                }
            }
        }
...
impl Worker {
    ...
    fn handle(&mut self, command: ClientCommand) {
        ...
        match command {
            ...
            ClientCommand::Set {
                ...
                let _ = response.send(
                    self.request("set", &name, &caller, Some(value))
                        .map(|_| ()),
                );
            }
        }
    ...
    fn connection_is_dead(&mut self) -> bool {
        let Some(c) = self.connection.as_mut() else {
            return true;
        };
